...
import stylesCentury from '@styles/century.module.scss'
...
const Century: NextPage<CenturyPageProperties> = ({ currentSeries, data, loanPicList, prefix, draftMode }) => {
  ...
  // const [showIChat, setShowIChat] = useState(false)
  ...
  useLayoutEffect(() => {
    ...
    const part1TL = ScrollTrigger.create({
      ...
      onToggle: self => self.isActive && setFbiSubNavActive(0),
      onUpdate:(self)=>{
        const progress = Number(self.progress.toFixed(2)) * 100;
        if(trackName && progress > 1 && progress < 100){
          trackName = false
          trackPv(`车型页-${currentSeries.displayName || currentSeries.name}-${fbiData[0]?.name}`)
        }
    ...
    const part2TL = ScrollTrigger.create({
      ...
      onToggle: self => self.isActive && setFbiSubNavActive(1),
      onUpdate:(self)=>{
        const progress = Number(self.progress.toFixed(2)) * 100;
        if(trackName && progress > 1 && progress < 100){
          trackName = false
          trackPv(`车型页-${currentSeries.displayName || currentSeries.name}-${fbiData[1].name}`)
        }
        if(progress === 100 || progress <= 0){
          trackName = true;
        }
    ...
    const part3TL = ScrollTrigger.create({
      ...
      onToggle: self => self.isActive && setFbiSubNavActive(2),
      onUpdate:(self)=>{
        const progress = Number(self.progress.toFixed(2)) * 100;
        if(trackName && progress > 1 && progress < 100){
          trackName = false
          trackPv(`车型页-${currentSeries.displayName || currentSeries.name}-${fbiData[2].name}`)
        }
        if(progress === 100 || progress <= 0){
          trackName = true;
        }
    ...
    const part4TL = ScrollTrigger.create({
      ...
      onToggle: self => self.isActive && setFbiSubNavActive(3),
      onUpdate:(self)=>{
        const progress = Number(self.progress.toFixed(2)) * 100;
        if(trackName && progress > 1 && progress < 100){
          trackName = false
          trackPv(`车型页-${currentSeries.displayName || currentSeries.name}-${fbiData[3].name}`)
        }
        if(progress === 100 || progress <= 0){
          trackName = true;
        }
  ...
  // const handleScrollTestdrive = useCallback(() => {
  //   if (!showTd) {
  //     setShowTd(true)
  //     setDisableScroll(false)
  //     setTimeout(setShowTd, 3000, false)
  //   }
  // }, [showTd])
  ...
  return (
    ...
    <main className={stylesCentury.main}>
      <SeriesNavigation
        ...
        prefix={assetsPrefix}
      />
      <AsideButtons tracking={currentSeries.displayName || currentSeries.name} onTop={handleScrollTop} hideTd={inView} showTop={!disableScroll && undefined} />
      ...
      <div className={classNames(stylesCentury['fbi-subnav'], {
        ...
        onClick={()=>{
          trackEvent(`车型页-${currentSeries.displayName || currentSeries.name}-一级页面-${res.name}`)
      ...
      <div ref={el => {
        ...
        <div className={styles.container}>
          <h4>预约试驾</h4>
          <Testdrive styles={styles} series={currentSeries} tracking={`车型页-${currentSeries.displayName || currentSeries.name}`} />
        </div>
      </div>

      <Overlay prefix={assetsPrefix} type={type} show={type ? true : false} data={data.century?.features} onClose={() => setType('')} />
